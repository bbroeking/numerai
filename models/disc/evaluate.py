...
from models.disc.data_preparation import prepare_data
...
MODEL_FILE = Path("model_disc.pickle.dat")
...
def main():
    ...
    training_data, tournament_data = prepare_data(ft_corr_list='disc_features1.pkl')
    ...
    tournament_data[PREDICTION_NAME].to_csv(f"../../submissions/disc/submission_{current_round}.csv", header=True)

    # Neutralize
    neutralized = tournament_data.copy()
    neutralized.loc[:, PREDICTION_NAME] = neutralize_series(tournament_data[PREDICTION_NAME], 
                                                        example_preds, 0.25)
    neutralized[PREDICTION_NAME].to_csv(f"../../submissions/disc/submission_{current_round}_neutralized.csv", header=True)
